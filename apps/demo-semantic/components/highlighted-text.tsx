 ...
 *
 * Only ever applied to lexical matches. There is nothing to underline in a semantic hit —
 * the words that made it match are, by construction, not the words on the page — and
 * pretending otherwise would misrepresent what the mode did.
 */
...
/**
 * `Highlight#trim` crops the text around its first match but hands back HTML. The marks
 * are stripped out again here — they get re-applied from `positions` at render time,
 * against a plain string, so the app never needs `dangerouslySetInnerHTML`.
 */
