...
We split the implementation into four published npm packages (currently **v3.3.1**) so the Cloudflare adapter stays thin:
...
flowchart LR
  subgraph runtime [Cloudflare Runtime]
    W["@zbsearch/runtime-cloudflare<br/>~85 lines"]
    R2A[R2ObjectStorage]
  ...
  subgraph core [Portable Core]
    EC["@zbsearch/edge-core"]
    EC --> ZB[zbsearch]
    EC --> MP[msgpack]
  end

  subgraph builder [External Builder]
    CLI["@zbsearch/edge-index-builder"]
    S3["@zbsearch/storage-s3"]
    CLI --> S3
    CLI --> EC
  end
...
| **External CLI / CI** | `npx zbsearch-edge-builder rebuild` against R2 via S3 API | Production, large indexes |
...
```bash
npm install @zbsearch/edge-index-builder@3.3.1
npx zbsearch-edge-builder rebuild --all
```

A typical CI job only needs Node.js, your R2 credentials as secrets, and the published builder package. No monorepo clone or local build step:

```yaml
- uses: actions/setup-node@v4
  with:
    node-version: 22
- run: npm install @zbsearch/edge-index-builder@3.3.1
- run: npx zbsearch-edge-builder rebuild --all
  env:
...
Schedule it every 15 minutes, trigger it manually, or invoke it from a webhook relay via `repository_dispatch`.

## Bulk import: skip the WAL entirely

For large catalogs, per-document API writes are the wrong tool. Install `@zbsearch/edge-index-builder` and use the `import` command to build a snapshot locally and upload it directly to R2:
...
```mermaid
flowchart LR
  JSON[catalog.json] --> CLI["zbsearch-edge-builder import"]
...
Everything ships on npm. Setup and teardown CLIs live inside `@zbsearch/runtime-cloudflare`; production rebuilds use `@zbsearch/edge-index-builder`. Install from any empty project directory:

```bash
mkdir my-search-api && cd my-search-api
npm init -y
npm install @zbsearch/runtime-cloudflare@3.3.1 wrangler
npx wrangler login
...
The generated `wrangler.toml` points at the Worker entrypoint inside `node_modules/@zbsearch/runtime-cloudflare`. You never compile packages locally.
...
For production rebuilds, add `@zbsearch/edge-index-builder@3.3.1` and run `npx zbsearch-edge-builder rebuild --all` from CI or your laptop.
