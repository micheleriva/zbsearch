...
Apply multiple operations in one request. All operations are written to a **single WAL file** (one R2 PUT).
...
Merge pending WAL operations into a new immutable snapshot and promote it to `liveVersion`.
