...
Cloudflare Workers and R2 have platform-level limits. The Worker `cpu_ms` limit in `wrangler.toml` is set to 300000 (5 minutes) to accommodate in-Worker rebuilds on small indexes. Large rebuilds should run outside the Worker.
