...
Warm single-request latency is ~180-450 ms end to end depending on shard count; engine time is tens of ms - the rest is network RTT and per-shard `meta.json` reads from R2 (see the [cost calculator](/docs/cloudflare/cost-calculator) for what those reads cost at scale).
