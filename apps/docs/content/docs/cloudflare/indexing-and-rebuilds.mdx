...
description: How ZBSearch Edge buffers writes, builds immutable snapshots in R2, and makes documents searchable. Cloudflare deploy is in beta.
---

ZBSearch Edge separates **writes** from **reads** on purpose. Search queries never merge a live write buffer at query time - they always load one immutable snapshot. This keeps latency predictable at the edge.
...
wal/{indexId}/entries/{ts}_{changeId}.ndjson
...
```
...
3. Read and apply all buffered operations (upserts overwrite, deletes remove)
4. Build a new ZBSearch database in memory
5. Serialize to msgpack and upload to R2
6. Clear the buffer
7. Set `liveVersion`, `documents`, `indexSizeBytes`, status `ready`, `pendingOps: 0`

```
Before rebuild:
  liveVersion: v1  (search reads this)
  buffer: [upsert doc-A, delete doc-B, upsert doc-C]
...
Let writes accumulate in the buffer. Rely on cron or CI to rebuild periodically. Monitor `pendingOps` via `/status`.
