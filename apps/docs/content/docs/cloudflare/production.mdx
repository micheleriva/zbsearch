...
| --- | --- | --- |
| &lt; 10k docs, &lt; 15 MB snapshot | In-Worker cron OK | ~470 req/s per isolate in local tests; med 1.1 ms, p95 16.5 ms warm |
...
| 100k+ docs / snapshot &gt; ~100 MB | External CLI; shard the index (`--shards N`) | Scatter-gather across shards fans out automatically; size shards so each snapshot stays well under `SNAPSHOT_CACHE_MAX_BYTES` |

v1 stores **full msgpack snapshots per shard**. A monolithic snapshot approaches the 128 MB isolate limit well before query volume becomes a problem — that is the ceiling the measured baseline in [Benchmarks](/docs/cloudflare/benchmarks) demonstrates, and [sharded indexes](/docs/cloudflare/api-reference#sharded-indexes) are the supported way past it. As a rule of thumb, at ~1.3 KB/document a shard of **25–50k documents** keeps each snapshot comfortably under the default 64 MB `SNAPSHOT_CACHE_MAX_BYTES`, which also bounds the re-indexing cost of dirty searches (WAL overlay) per shard. Very large indexes will hit Worker memory limits during in-Worker rebuild — use the external builder and watch `indexSizeBytes` in `/status`.
