...
The default `wrangler.toml` (and the setup-CLI template) also includes the Durable Object binding for the write coordinator and an extended CPU limit:
...
<Callout type="warn">
  Durable Objects and custom `cpu_ms` limits both require the **Workers Paid plan**. On the Free plan, remove these blocks — edge-core falls back to per-op WAL entries with the default CPU budget. See [Production](/docs/cloudflare/production#cloudflare-plan-requirements).
</Callout>
