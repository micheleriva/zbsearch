...
If you are developing ZBSearch itself, clone the repository and use the same CLIs via pnpm:
...
## 3. Configure the rebuild CLI (optional but recommended)

The Worker handles search and buffered writes. **Rebuilds** that merge the buffer into a new snapshot can run inside the Worker (small indexes) or via an external CLI (production). The CLI uses R2's S3-compatible API.
...
<Callout type="info">
  The Worker binds to R2 natively via Wrangler - it does **not** need these S3 credentials. They are only for the rebuild CLI and CI runners.
</Callout>
...
Insert a document (buffered - not searchable yet):
...
Then redeploy with `pnpm deploy:edge`.
...
Monorepo equivalent: `pnpm teardown:edge -- --yes`
...
| Worker CPU timeout on rebuild | Index too large for in-Worker rebuild | Use external builder via `BUILDER_WEBHOOK_URL` |
