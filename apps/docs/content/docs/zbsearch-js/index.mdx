...
## [Performance vs Orama](/docs/zbsearch-js/vs-orama)

ZBSearch is a drop-in fork of Orama with the same API and persistence format, but with heavily optimized internals. We benchmark both engines head-to-head in the [`benchmarks/`](https://github.com/micheleriva/zbsearch/tree/main/benchmarks) suite (Orama **3.1.18** vs ZBSearch **3.2.1**, July 2026). The headline: **ZBSearch is faster on almost every query path, with no trade-off in indexing speed or on-disk size.** Full results are on the [ZBSearch vs Orama](/docs/zbsearch-js/vs-orama) page.
...
For more information, check out the [Usage](/docs/zbsearch-js/usage/create) section.
