---
title: Components
description: Learn how to customize ZBSearch by using its components architecture.
---
ZBSearch can be completely customized and extended by using its components architecture.
...
synchronous or return a promise and ZBSearch will make sure everything is handled correctly.

When no components are specified, an ZBSearch database is created with some defaults components which satisfies most common use cases:
...
The tokenizer is used to tokenize documents fields and search terms. To customize the tokenizer used by ZBSearch, provide an object which has at least the following properties:
...
The ZBSearch's default tokenizer is exported via `zbsearch/components` and can be customized:

```javascript
import { create } from "zbsearch";
import { tokenizer as defaultTokenizer } from "zbsearch/components";
...
The index component is used to perform the indexing and searching of documents in ZBSearch.

To customize the index used by ZBSearch, provide an object which has at least the following properties:

- `create`: A function that creates a new index. It receives the following arguments:
  - `orama`: The ZBSearch instance.
...
- `searchByWhereClause`: A function that searches in boolean and numeric indexes and returns a list of matching IDs. It receives the following arguments:
  - `context`: A search context with various useful information about the search.
  - `index`: The index.
  - `filters`: An object where keys are the properties to match and the values are search operators as described in the [filters](/docs/zbsearch-js/search/filters) page.
...
For the more formal interface information, look for the `IIndex` interface in `src/types.ts` in ZBSearch's source code.

The ZBSearch's default index is based on BM25, Radix Trees and AVL trees. All its functions are exported via `zbsearch/components` and can be composed to create a custom index:

```javascript
import { create } from "zbsearch";
import { index as defaultIndex } from "zbsearch/components";
...
The documentsStore component is used to store the documents in ZBSearch.

To customize the documents store used by ZBSearch, provide an object which has at least the following properties:
...
For the more formal interface information, look for the `IDocumentsStore` interface in `src/types.ts` in ZBSearch's source code.

The ZBSearch's default documents store is based on simple JavaScript object. All its functions are exported via `zbsearch/components` and can be composed to create a custom documents store:

```javascript
import { create } from "zbsearch";
import { documentsStore as defaultDocumentsStore } from "zbsearch/components";
...
The sorter component is used to store the documents in ZBSearch.

To customize the documents sort used by ZBSearch, provide an object which has at least the following properties:
...
```javascript
import { create } from "zbsearch";
import { sorter as defaultSorter } from "zbsearch/components.js";
...
The components in this category are simple functions which are internally used by ZBSearch.
Depending on the use case the component must return a value. ZBSearch will await if a Promise is returned.
...
In order to improve performance, ZBSearch uses an internal ID for each document. The documents IDs mapper component is used to maintain a between the user document ID and the ZBSearch document ID.
...
When writing or reading documents in the `data` section of The ZBSearch instance, such as `orama.data.index`, make sure you always use a internal ID.

ZBSearch exports two helpers which will help dealing with this operations:
...
As ZBSearch is an Open Source Software, we gladly accept proposal for new functionalities.

This obviously include the definition of new components which are used internally by ZBSearch and that can be customized by the users.
...
Remember that all functions used in ZBSearch's components can be async, so we advise to use the `SyncOrAsyncValue` for their return value.
...
export async function create({ schema, language, components }: CreateArguments): Promise<ZBSearch> {
...
```diff
export async function search(orama: ZBSearch, params: SearchParams, language?: string): Promise<Results> {
