---
title: Plugin Parsedoc
description: Learn how to use the Parsedoc plugin in ZBSearch.
---
The plugin-parsedoc plugin allows ZBSearch to parse and create indexes from HTML and Markdown documents automatically.
...
```bash
npm install @zbsearch/plugin-parsedoc
```
...
import { populateFromGlob, defaultHtmlSchema } from "@zbsearch/plugin-parsedoc";
...
- `db`: the database to populate. Should use ZBSearch's native `insert` or `insertBatch` methods internally.
...
```ts copy
import type { populateFromGlob } from "@zbsearch/plugin-data-parsedoc";
```
