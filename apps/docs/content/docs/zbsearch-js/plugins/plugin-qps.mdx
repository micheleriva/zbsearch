...
**QPS**, short for **Quantum Proximity Scoring**, is a new algorithm developed for ZBSearch that enhances document relevance by evaluating token proximity within a document.
...
```bash
npm install @zbsearch/plugin-qps
```
