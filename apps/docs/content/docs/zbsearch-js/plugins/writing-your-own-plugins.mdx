...
With ZBSearch `v2.0.0-beta.5`, we introduced the new plugin system. This allows you to write your own plugins and use them in your ZBSearch project.
...
function loggerPlugin() {
  return {
    ...
    beforeSearch: (orama, query) => {
      console.log(
        `About to search for ${query.term} on ZBSearch instance ${orama.id}`
      );
    },
  };
}
...
async function afterInsertPluginExample() {
  return {
    ...
    afterInsert: async (orama, id, document) => {
      console.log("ZBSearch instance: ", orama);
...
With `v2.0.0-beta.5`, we essentially moved the hooks from the `components` property of the ZBSearch instance to the `plugins` property.
