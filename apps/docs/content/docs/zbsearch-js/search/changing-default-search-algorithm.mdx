...
Since version `3.0.0`, ZBSearch allows you to change the default search algorithm (**BM25**) with two new plugins: **QPS** (Quantum Proximity Scoring) and **PT15** (Positional Token 15).
...
To change the default search algorithm, you can install the **QPS** or **PT15** plugin in your ZBSearch instance:
...
And that's it! The ZBSearch plugin will do the rest for you.
...
QPS (Quantum Proximity Scoring) is a search algorithm developed by the ZBSearch team in 2024. It was designed to improve search performance, particularly in browser-based environments and memory-constrained systems.
...
PT15 (Positional Token 15) is a search algorithm developed by the ZBSearch team in 2024, inspired by Thomas Wilkerling's work on Flexsearch.
...
At ZBSearch, we developed **QPS** after a long process of evaluation and observation on the usage of **BM25**.
...
If you want to know what search queries do your users perform, you can install the [Search Analytics plugin](/docs/zbsearch-js/plugins/plugin-analytics) to track and analyze search queries in your ZBSearch instance, for free.