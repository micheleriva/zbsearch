---
title: Geosearch
description: Learn how to perform geosearch queries in ZBSearch.
---
...
To perform geosearch queries, you first have to define a new `geopoint` property inside your schema definition when creating your ZBSearch instance:
...
To avoid confusion, please note that ZBSearch is a full-text and vector search library. We do not provide the tools to draw on maps.
...
If we follow the configuration above, we are asking ZBSearch to return all the documents that are within 100 meters from the Gallery, as shown in the following map:
...
ZBSearch currently supports the following units of measurement:
...
All these units will be converted into meters automatically. If you feel like we should support other units of measurement, please [open an issue](https://github.com/micheleriva/zbsearch/issues)
...
<Iframe src="https://www.google.com/maps/d/u/1/embed?mid=1RbUtX13X4WXI4VH46P2nuwomvkxk7Ds&ehbc=2E312F&noprof=1" width="100%" style="height: 480px" />
...
By default, ZBSearch will use the Haversine Formula to calculate the distance between two points.
...
Even though the Haversine Formula is more than enough for most use cases, if you need to perform geosearch queries with higher precision, you can force ZBSearch to use the **Vincenty Formula** by setting `highPrecision: true` when performing your search:
