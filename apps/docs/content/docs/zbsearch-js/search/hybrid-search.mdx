---
title: Hybrid Search
description: Learn how to perform hybrid search in ZBSearch.
---
Hybrid search is an ZBSearch feature that allows you to perform full-text and vector search in one unique query, combining the results to get the best of both worlds.

<Callout title="Generating Embeddings">
In this page, we will assume that you already have the embeddings for your documents.

If you don't, you can use the [Plugin Embeddings](/docs/zbsearch-js/plugins/plugin-embeddings) to generate them directly offline and on-device at insert and search time.

If you're running ZBSearch in a memory-constrained environment, you can also use the [Plugin Secure Proxy](/docs/zbsearch-js/plugins/plugin-secure-proxy) to generate embeddings on the fly using the OpenAI API directly from the client-side.
</Callout>

## Performing Hybrid Search

To perform hybrid search, you will need to use the same `search` method you're already using for full-text and vector search, which can be imported from `zbsearch`:
...
4. At the time of writing, you can only search through one vector property at a time. If you think that this is too limiting, please open a [feature request](https://github.com/micheleriva/zbsearch/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=) to support multiple vector properties at search-time.
...
When running vector search using the [secure proxy](/docs/zbsearch-js/plugins/plugin-embeddings) or [embeddings](/docs/zbsearch-js/plugins/plugin-secure-proxy) plugins, you won't need to explicitly pass the `vector` configuration. Just run a simple search query!
...
By default, ZBSearch performs full-text and vector searches concurrently. Then, it aggregates the results and uses default weights to determine which result is more important (the full-text or the vector one) in the final list of results.
...
By default, ZBSearch assigns `0.5` to `text` and `0.5` to `vector`. This means that full-text results and vector results carry the same weight.
