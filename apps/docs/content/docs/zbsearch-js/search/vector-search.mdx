---
title: Vector Search
description: Learn how to perform vector search using ZBSearch.
---
Being a vector database, ZBSearch allows you to perform vector search natively.

To perform search through vectors, you need to correctly configure your ZBSearch schema, as described in the [create page](/docs/zbsearch-js/usage/create).
...
To perform vector search, you will need to use the same `search` method you're already using for full-text and hybrid search, which can be imported from `zbsearch`:
...
3. At the time of writing, you can only search through one vector property at a time. If you think that this is too limiting, please open a [feature request](https://github.com/micheleriva/zbsearch/issues/new?assignees=&labels=&projects=&template=feature_request.md&title=) to support multiple vector properties at search-time.
