...
It is the shared core behind the [Docusaurus](/docs/zbsearch/integrations/docusaurus),
[Starlight](/docs/zbsearch/integrations/starlight) and [VitePress](/docs/zbsearch/integrations/vitepress)
plugins. Reach for it directly when you are wiring ZBSearch into a framework none of those covers.
...
An inline payload grows with your content, and past a certain point shipping all of it to search any of it
stops being a good deal. `buildIndexAuto` draws that line for you:

```typescript copy
import { buildIndexAuto } from '@zbsearch/docs-index/node';

const { payload, payloadJson, staticFiles } = await buildIndexAuto(records, 'english', {
  baseUrl: '/zbsearch-static/',
});
```

Below the limit - 256 KB of JSON by default, tunable with `inlineLimitBytes` - the result is the familiar
single-file payload, and `staticFiles` is absent. Above it, the index is built as a
[sharded static file set](/docs/zbsearch/integrations/static) instead: `staticFiles` holds the artifacts to
write under `baseUrl`, and `payload` shrinks to a sentinel of about a hundred bytes that tells the browser
where to find them.

The browser side needs no second code path: `createIndexLoader` and `createSearcher` accept either payload,
and a sharded one makes the searcher fetch postings and documents lazily per query - with rankings identical
to the inline index.

`shardBuiltPayload` does the same for integrations that only see the already-serialized payload, such as a
prerendered route in a finished build. It recovers the records from the payload's own documents store, so
nothing needs to be re-collected:

```typescript copy
import { shardBuiltPayload } from '@zbsearch/docs-index/node';

const result = await shardBuiltPayload(await readFile('dist/zbsearch-index.json', 'utf8'), {
  baseUrl: '/zbsearch-static/',
});
```
...
[`@zbsearch/searchbox-react`](/docs/zbsearch/integrations/searchbox-react) and
[`@zbsearch/searchbox-vue`](/docs/zbsearch/integrations/searchbox-vue) both expect, so they drop together.
...
| `@zbsearch/docs-index/node` | `parseMarkdown`, `stripInlineMarkup`, `buildIndex`, `buildIndexAuto` and `shardBuiltPayload` |
