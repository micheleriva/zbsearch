...
If you don't, you can use the [Plugin Embeddings](/docs/zbsearch/plugins/plugin-embeddings) to generate them directly offline and on-device at insert and search time.

If you're running ZBSearch in a memory-constrained environment, you can also use the [Plugin Secure Proxy](/docs/zbsearch/plugins/plugin-secure-proxy) to generate embeddings on the fly using the OpenAI API directly from the client-side.
</Callout>
...
When running vector search using the [secure proxy](/docs/zbsearch/plugins/plugin-embeddings) or [embeddings](/docs/zbsearch/plugins/plugin-secure-proxy) plugins, you won't need to explicitly pass the `vector` configuration. Just run a simple search query!
