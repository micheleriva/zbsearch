...
ZBSearch is a fork of Orama maintained by the original engineering team. The API is compatible, but the internals have been optimized - including an explicit **postings list** inverted index that lowers memory use and serialized bundle size while keeping search fast, plus a faster full-text insert path. This page summarizes benchmark results from the [`benchmarks/`](https://github.com/micheleriva/zbsearch/tree/main/benchmarks) suite in this repository.

<Callout title="Test environment">
Results below were collected using **Orama 3.1.18** and **ZBSearch 3.3.1**, run via [Benny](https://github.com/caderek/benny) for throughput and isolated Node.js processes (`--expose-gc`) for memory. Search and secondary suites were measured on **July 10, 2026**; full-text indexing was remeasured on **July 20, 2026** after insert-path optimizations. Higher **ops/s** is better. Percentages show how much slower the loser is relative to the winner in each suite.
</Callout>
...
| Area | Verdict |
| --- | --- |
| Full-text search | ZBSearch is **11–58% faster** on search; indexing is **~42–44% faster** |
...
| Benchmark | Orama 3.1.18 | ZBSearch 3.3.1 | Difference |
| --- | ---: | ---: | --- |
| Insert | 19 ops/s | 33 ops/s | ZBSearch **42.4%** faster |
| Insert multiple | 18 ops/s | 32 ops/s | ZBSearch **43.8%** faster |
...
ZBSearch is faster across all search scenarios tested here, with the largest gap on long-text queries with complex filters. Indexing is also substantially faster (~**1.7–1.8×** Orama) thanks to node-local postings caches and a tighter tokenize/insert path.
