...
export interface ZBSearchLogoProps {
  className?: string
  size?: number
  monochrome?: boolean
}

export function ZBSearchLogo({ className, size = 14, monochrome = false }: ZBSearchLogoProps) {
  const gradientId = `zbs-logo-gradient-${useId()}`
      ...
      {monochrome ? null : (
        <defs>
          <radialGradient
            ...
            cy="0"
            r="1"
            gradientTransform="matrix(-0.33041 235.871 -291.438 -0.406356 157.33 15.1292)"
            gradientUnits="userSpaceOnUse"
          >
            <stop stopColor="#FD00EF" />
            <stop offset="0.585385" stopColor="#B500F1" />
            <stop offset="1" stopColor="#690C70" />
          </radialGradient>
        </defs>
      )}
    </svg>
