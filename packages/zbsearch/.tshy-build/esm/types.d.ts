...
export interface SearchParamsFullText<T extends AnyZBSearch, ResultDocument = TypedDocument<T>> extends SearchParamsBase<T, ResultDocument> {
    /**
...
export interface SearchParamsHybrid<T extends AnyZBSearch, ResultDocument = TypedDocument<T>> extends SearchParamsBase<T, ResultDocument> {
    /**
...
export interface SearchParamsVector<T extends AnyZBSearch, ResultDocument = TypedDocument<T>> extends SearchParamsBase<T, ResultDocument> {
    /**
     * Search mode. Tell ZBSearch to perform either a fulltext search, a vector search or a hybrid search.
     * By default, ZBSearch will perform a full-text search.
     */
...
export type SearchParams<T extends AnyZBSearch, ResultDocument = TypedDocument<T>> = SearchParamsFullText<T, ResultDocument> | SearchParamsHybrid<T, ResultDocument> | SearchParamsVector<T, ResultDocument>;
...
export type SearchContext<T extends AnyZBSearch, ResultDocument = TypedDocument<T>, P = SearchParams<T, ResultDocument>> = {
    timeStart: bigint;
    tokenizer: Tokenizer;
    index: T['index'];
 ...
 * ```
 */
export type SingleCallbackComponent<T extends AnyZBSearch> = (zbsearch: T, id: string, doc?: TypedDocument<T>) => SyncOrAsyncValue;
/**
 * Sometimes {@link doc} will not have the correct type; in these cases,
 * you can simply create a new variable and convert it to the correct type like:
 *
 ...
 * ```
 */
export type MultipleCallbackComponent<T extends AnyZBSearch> = (zbsearch: T, doc: TypedDocument<T>[] | string[]) => SyncOrAsyncValue;
/**
 ...
 * ```
 */
export type AfterSearch<T extends AnyZBSearch, ResultDocument extends TypedDocument<T> = TypedDocument<T>> = (db: T, params: SearchParams<T, ResultDocument>, language: string | undefined, results: Results<ResultDocument>) => SyncOrAsyncValue;
export type BeforeSearch<T extends AnyZBSearch> = (db: T, params: SearchParams<T>, language: string | undefined) => SyncOrAsyncValue;
...
export interface IIndex<I extends AnyIndexStore> {
    create<T extends AnyZBSearch>(zbsearch: T, sharedInternalDocumentStore: T['internalDocumentIDStore'], schema: T['schema']): I;
...
export interface ISorter<So extends AnySorterStore> {
    create<T extends AnyZBSearch>(zbsearch: T, sharedInternalDocumentStore: InternalDocumentIDStore, schema: T['schema'], sorterConfig?: SorterConfig): So;
...
export type Components<T extends AnyZBSearch, TSchema, TIndex, TDocumentStore, TSorter, TPinning> = Partial<ObjectComponents<TIndex, TDocumentStore, TSorter, TPinning> & FunctionComponents<TSchema> & SingleOrArrayCallbackComponents<T>>;
...
type Internals<TSchema, TIndex extends AnyIndexStore, TDocumentStore extends AnyDocumentStore, TSorter extends AnySorterStore, TPinning extends AnyPinningStore> = {
    version: string;
    schema: TSchema;
...
export type AnyGenericPinning<T> = T extends IPinning<infer TPinning> ? TPinning extends AnyPinningStore ? TPinning : never : AnyPinningStore;
export type PickInferGeneric<T, Default> = T extends AnyGeneric<infer Generic> ? Generic extends Default ? Generic : never : never;
export type ZBSearch<TSchema, TIndex = IIndex<Index>, TDocumentStore = IDocumentsStore<DocumentsStore>, TSorter = ISorter<Sorter>, TPinning = IPinning<AnyPinningStore>> = FunctionComponents<TSchema> & Internals<TSchema, AnyGenericIndex<TIndex>, AnyGenericDocumentStore<TDocumentStore>, AnyGenericSorter<TSorter>, AnyGenericPinning<TPinning>> & ArrayCallbackComponents<any> & ZBSearchID & {
    plugins: ZBSearchPlugin[];
};
export type AnyZBSearch<TSchema = any> = FunctionComponents<TSchema> & Internals<TSchema, AnyIndexStore, AnyDocumentStore, AnySorterStore, AnyPinningStore> & ArrayCallbackComponents<any> & ZBSearchID & {
    plugins: ZBSearchPlugin[];
};
export type ZBSearchPluginSync<T = unknown> = {
    ...
    beforeSearch?: <T extends AnyZBSearch>(zbsearch: T, params: SearchParams<T>, language: string | undefined) => SyncOrAsyncValue;
    afterSearch?: <T extends AnyZBSearch>(zbsearch: T, params: SearchParams<T>, language: string | undefined, results: Results<TypedDocument<T>>) => SyncOrAsyncValue;
    ...
    getComponents?: <IndexStore extends AnyIndexStore, TDocumentStore, TSorter, TPinning>(schema: AnySchema) => SyncOrAsyncValue<Partial<ObjectComponents<IIndex<IndexStore>, TDocumentStore, TSorter, TPinning>>>;
};
