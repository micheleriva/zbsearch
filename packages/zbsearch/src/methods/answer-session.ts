import type { AnyDocument, AnyZBSearch, Nullable, ZBSearchPluginSync, SearchParams, Results } from '../types.js'
...
export type Interaction<SourceT = AnyDocument> = {
  ...
  translatedQuery: Nullable<SearchParams<AnyZBSearch>>
  error: boolean
...
const ZBSEARCH_SECURE_PROXY_PLUGIN_NAME = 'zbsearch-secure-proxy'

export class AnswerSession<SourceT = AnyDocument> {
  private db: AnyZBSearch
  private proxy: Nullable<ZBSearchProxy> = null
  ...
  constructor(db: AnyZBSearch, config: IAnswerSessionConfig<SourceT>) {
    this.db = db
    this.config = config
  ...
  private async init(): Promise<void> {
    ...
    async function getPlugin() {
      return await self.db.plugins.find((plugin) => (plugin as ZBSearchPluginSync).name === ZBSEARCH_SECURE_PROXY_PLUGIN_NAME)
    }
    ...
    const pluginExtras = plugin.extra as { proxy: ZBSearchProxy; pluginParams: { chat: { model: ChatModel } } }
